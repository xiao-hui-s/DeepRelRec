...
fname = now.strftime('%Y-%m-%d-%H-%M-%S') + ' user-item' + '.txt'
...
def parse_args():
    ...
    parser.add_argument('--reg_t', type=int, default=0.01,
...
class MF(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            self.ru1_p = tf.placeholder(tf.int32, shape=[None, None])
            self.cu1_p = tf.placeholder(tf.float32, shape=[None])
            self.eu1_p = tf.placeholder(tf.float32, shape=[None, None])
            self.lenu1_p = tf.placeholder(tf.int32)

            self.ru2_p = tf.placeholder(tf.int32, shape=[None, None])
            self.cu2_p = tf.placeholder(tf.float32, shape=[None])
            self.eu2_p = tf.placeholder(tf.float32, shape=[None, None])
            self.lenu2_p = tf.placeholder(tf.int32)

            self.ru3_p = tf.placeholder(tf.int32, shape=[None, None])
            self.cu3_p = tf.placeholder(tf.float32, shape=[None])
            self.eu3_p = tf.placeholder(tf.float32, shape=[None, None])
            self.lenu3_p = tf.placeholder(tf.int32)

            self.ru1_n = tf.placeholder(tf.int32, shape=[None, None])
            self.cu1_n = tf.placeholder(tf.float32, shape=[None])
            self.eu1_n = tf.placeholder(tf.float32, shape=[None, None])
            self.lenu1_n = tf.placeholder(tf.int32)

            self.ru2_n = tf.placeholder(tf.int32, shape=[None, None])
            self.cu2_n = tf.placeholder(tf.float32, shape=[None])
            self.eu2_n = tf.placeholder(tf.float32, shape=[None, None])
            self.lenu2_n = tf.placeholder(tf.int32)

            self.ru3_n = tf.placeholder(tf.int32, shape=[None, None])
            self.cu3_n = tf.placeholder(tf.float32, shape=[None])
            self.eu3_n = tf.placeholder(tf.float32, shape=[None, None])
            self.lenu3_n = tf.placeholder(tf.int32)
            ...
            self.ru1_embedding = tf.expand_dims(tf.nn.embedding_lookup(self.weights['relation_type_user_embeddings'], 0), axis=0)  # [1,H]
            self.ru2_embedding = tf.expand_dims(tf.nn.embedding_lookup(self.weights['relation_type_user_embeddings'], 1), axis=0)  # [1,H]
            self.ru3_embedding = tf.expand_dims(tf.nn.embedding_lookup(self.weights['relation_type_user_embeddings'], 2), axis=0)  # [1,H]
            ...
            # item's attenion for positive relation type
            self.relation_type_user_embedding = tf.expand_dims(self.weights['relation_type_user_embeddings'], axis=0)  # [1,3,H]
            self.pos_u_embedding = tf.expand_dims(self.pos_embedding, axis=1)
            self.type_product_u_p = tf.multiply(self.pos_u_embedding, self.relation_type_user_embedding)  # [B,3,H]
            self.type_w_u_p = tf.layers.dense(self.type_product_u_p, self.attention_size, name='first_layer_attention_type_user_p',
                                          reuse=tf.AUTO_REUSE, use_bias=True, activation=tf.nn.relu)  # [B,3,A]
            self.logits_type_u_p = tf.reduce_sum(
                tf.layers.dense(self.type_w_u_p, 1, name='final_layer_attention_type_user_p', reuse=tf.AUTO_REUSE, use_bias=False),
                axis=-1)  # [B,3]
            self.logits_exp_type_u_p = tf.exp(self.logits_type_u_p)  # [B,3]
            self.exp_sum_type_u_p = tf.reduce_sum(self.logits_exp_type_u_p, axis=-1, keep_dims=True)  # [B,1]
            self.attention_type_u_p = self.logits_exp_type_u_p / self.exp_sum_type_u_p  # [B,3]

            # item's attenion for negative relation type
            self.relation_type_user_embedding = tf.expand_dims(self.weights['relation_type_user_embeddings'], axis=0)  # [1,3,H]
            self.neg_u_embedding = tf.expand_dims(self.neg_embedding, axis=1)
            self.type_product_u_n = tf.multiply(self.neg_u_embedding, self.relation_type_user_embedding)  # [B,3,H]
            self.type_w_u_n = tf.layers.dense(self.type_product_u_n, self.attention_size, name='first_layer_attention_type_n',
                                          reuse=tf.AUTO_REUSE, use_bias=True, activation=tf.nn.relu)  # [B,3,A]
            self.logits_type_u_n = tf.reduce_sum(
                tf.layers.dense(self.type_w_u_n, 1, name='final_layer_attention_type_user_n', reuse=tf.AUTO_REUSE, use_bias=False),
                axis=-1)  # [B,3]
            self.logits_exp_type_u_n = tf.exp(self.logits_type_u_n)  # [B,3]
            self.exp_sum_type_u_n = tf.reduce_sum(self.logits_exp_type_u_n, axis=-1, keep_dims=True)  # [B,1]
            self.attention_type_u_n = self.logits_exp_type_u_n / self.exp_sum_type_u_n  # [B,3]
            ...
            self.att0_j_p = tf.layers.dense(self.r0_p_embedding, self.attention_size, name='w0_j', reuse=tf.AUTO_REUSE,
                                            use_bias=True)  # [B,?,A]
            self.att0_sum_p = tf.add(self.att0_i_p, self.att0_j_p)  #[B,?,A]
            ...
            self.exp_0_p = tf.exp(self.att0_losits_p)  # [B,?]
            ...
            self.att1_e_p = tf.layers.dense(self.e1_p_embedding, self.attention_size, name='w1_e', reuse=tf.AUTO_REUSE,
                                            use_bias=True)  # [B,?,A]
            self.att1_sum_p = tf.add(self.att1_i_p, self.att1_j_p)  # [B,?,A]
            ...
            self.att1_losits_p = self.Mask(self.att1_losits_p, self.cnt1_p + 1, self.len1_p,
                                           mode='add')  # [B,?] Mask the filling positions
            self.exp_1_p = tf.exp(self.att1_losits_p)  # [B,?]
            ...
            self.att2_losits_p = self.Mask(self.att2_losits_p, self.cnt2_p + 1, self.len2_p,
                                           mode='add')  # [B,?] Mask the filling positions
            self.exp_2_p = tf.exp(self.att2_losits_p)  # [B,?]
            ...
            self.att3_e_p = tf.layers.dense(self.e3_p_embedding, self.attention_size, name='w3_e', reuse=tf.AUTO_REUSE,
                                            use_bias=True)  # [B,?,A]
            self.att3_sum_p = tf.add(self.att3_i_p, self.att3_j_p)  # [B,?,A]
            ...
            self.att3_losits_p = self.Mask(self.att3_losits_p, self.cnt3_p + 1, self.len3_p,
                                           mode='add')  # [B,?] Mask the filling positions
            self.exp_3_p = tf.exp(self.att3_losits_p)  # [B,?]
            ...
            self.ru1_p_embedding = tf.nn.embedding_lookup(self.weights['user_embeddings'], self.ru1_p)  # [B,?,H]

            self.att1_losits_u_p = tf.expand_dims(self.Mask(self.eu1_p, self.cu1_p + 1, self.lenu1_p,
                                           mode='mul'), axis=1)  # [B,?] Mask the filling positions
            self.user_latent_1_p = tf.matmul(self.att1_losits_u_p, self.ru1_p_embedding)  # [B,1,H]



            #ru2

            self.ru2_p_embedding = tf.nn.embedding_lookup(self.weights['user_embeddings'], self.ru2_p)  # [B,?,H]
            self.att2_losits_u_p = tf.expand_dims(self.Mask(self.eu2_p, self.cu2_p + 1, self.lenu2_p,
                                             mode='mul'), axis=1)  # [B,?] Mask the filling positions
            self.user_latent_2_p = tf.matmul(self.att2_losits_u_p, self.ru2_p_embedding)  # [B,1,H]



            #ru3

            self.ru3_p_embedding = tf.nn.embedding_lookup(self.weights['user_embeddings'], self.ru3_p)  # [B,?,H]
            self.att3_losits_u_p = tf.expand_dims(self.Mask(self.eu3_p, self.cu3_p + 1, self.lenu3_p,
                                             mode='mul'), axis=1)  # [B,?] Mask the filling positions
            self.user_latent_3_p = tf.matmul(self.att3_losits_u_p, self.ru3_p_embedding)  # [B,1,H]

            # merge all user latent in different relations
            self.user_latent_p = tf.concat(
                [self.user_latent_1_p, self.user_latent_2_p, self.user_latent_3_p],
                axis=1)  # [B,3,H]
            self.user_latent_p = tf.reduce_sum(
                tf.matmul(tf.expand_dims(self.attention_type_u_p, axis=1), self.user_latent_p), axis=1)  # [B,H]



            self.mu_p = tf.add(tf.reduce_sum(self.user_embedding, axis=1), self.user_latent_p)  # [B,H]
            # self.mi_p = tf.add(self.pos_embedding, self.user_latent_p)
            self.mp_u = tf.add(self.pos_embedding, self.item_latent_p)
            # self.pos = tf.reduce_sum(tf.multiply(self.mu_p, self.pos_embedding), 1)
            self.pos = tf.multiply(self.mu_p, self.mp_u)  # [B,H]
            self.pos = tf.nn.dropout(self.pos, self.dropout_keep[-1])  #[B,H]
            for i in range(0, len(self.layers)):
                self.pos = tf.add(tf.matmul(self.pos, self.weights['layer_%d' % i]),
                                  self.weights['bias_%d' % i])  # None * layer[i] * 1  #[B,1]
                self.pos = self.activation_function(self.pos)
                self.pos = tf.nn.dropout(self.pos, self.dropout_keep[i])  # dropout at each Deep layer
            self.pos = tf.matmul(self.pos, self.weights['prediction'])  # None * 1   #[B,1]
            ...
            self.att0_sum_n = tf.add(self.att0_i_n, self.att0_j_n)  # [B,?,A]
            ...
            self.att0_losits_n = self.Mask(self.att0_losits_n, self.cnt0_n + 1, self.len0_n,
                                           mode='add')  # [B,?] Mask the filling positions
            self.exp_0_n = tf.exp(self.att0_losits_n)  # [B,?]
            ...
            self.att1_e_n = tf.layers.dense(self.e1_n_embedding, self.attention_size, name='w1_e', reuse=tf.AUTO_REUSE,
                                            use_bias=True)  # [B,?,A]
            self.att1_sum_n = tf.add(self.att1_i_n, self.att1_j_n) # [B,?,A]
            ...
            self.att1_losits_n = self.Mask(self.att1_losits_n, self.cnt1_n + 1, self.len1_n,
                                           mode='add')  # [B,?] Mask the filling positions
            self.exp_1_n = tf.exp(self.att1_losits_n)  # [B,?]
            ...
            self.att2_e_n = tf.layers.dense(self.e2_n_embedding, self.attention_size, name='w2_e', reuse=tf.AUTO_REUSE,
                                            use_bias=True)  # [B,?,A]
            self.att2_sum_n = tf.add(self.att2_i_n, self.att2_j_n)  # [B,?,A]
            ...
            self.att2_losits_n = self.Mask(self.att2_losits_n, self.cnt2_n + 1, self.len2_n,
                                           mode='add')  # [B,?] Mask the filling positions
            self.exp_2_n = tf.exp(self.att2_losits_n)  # [B,?]
            ...
            self.ru1_n_embedding = tf.nn.embedding_lookup(self.weights['user_embeddings'], self.ru1_n)  # [B,?,H]
            self.att1_losits_u_n = tf.expand_dims(self.Mask(self.eu1_n, self.cu1_n + 1, self.lenu1_n,
                                             mode='mul'), axis=1)  # [B,?] Mask the filling positions
            self.user_latent_1_n = tf.matmul(self.att1_losits_u_n, self.ru1_n_embedding)  # [B,1,H]

            # ru2

            self.ru2_n_embedding = tf.nn.embedding_lookup(self.weights['user_embeddings'], self.ru2_n)  # [B,?,H]
            self.att2_losits_u_n = tf.expand_dims(self.Mask(self.eu2_n, self.cu2_n + 1, self.lenu2_n,
                                             mode='mul'), axis=1)  # [B,?] Mask the filling positions
            self.user_latent_2_n = tf.matmul(self.att2_losits_u_n, self.ru2_n_embedding)  # [B,1,H]

            # ru3

            self.ru3_n_embedding = tf.nn.embedding_lookup(self.weights['user_embeddings'], self.ru3_n)  # [B,?,H]
            self.att3_losits_u_n = tf.expand_dims(self.Mask(self.eu3_n, self.cu3_n + 1, self.lenu3_n,
                                             mode='mul'), axis=1)  # [B,?] Mask the filling positions
            self.user_latent_3_n = tf.matmul(self.att3_losits_u_n, self.ru3_n_embedding)  # [B,1,H]

            # merge all user latent in different relations
            self.user_latent_n = tf.concat(
                [self.user_latent_1_n, self.user_latent_2_n, self.user_latent_3_n],
                axis=1)  # [B,3,H]
            self.user_latent_n = tf.reduce_sum(
                tf.matmul(tf.expand_dims(self.attention_type_u_n, axis=1), self.user_latent_n), axis=1)  # [B,H]


            self.mu_n = tf.add(tf.reduce_sum(self.user_embedding, axis=1), self.user_latent_n)
            # self.mi_n = tf.add(self.neg_embedding, self.user_latent_n)
            self.mn_u = tf.add(self.neg_embedding, self.item_latent_n)
            # self.neg = tf.reduce_sum(tf.multiply(self.mu_n, self.neg_embedding), 1)
            self.neg = tf.multiply(self.mu_n, self.mn_u)
    ...
    def _initialize_weights(self):
        ...
        if self.pretrain_flag > 0:
            ...
            all_weights['relation_type_user_embeddings'] = tf.Variable(
                tf.random_normal([3, self.hidden_factor], 0.0, 0.05), name='relation_type_user_embeddings')
            ue = tf.Variable(tf.random_normal([self.num_users, self.hidden_factor], 0.0, 0.05), name='relation_type_embeddings')
            ...
            all_weights['user_embeddings'] = tf.concat([ue, mask_embedding], axis=0)
    ...
    def partial_fit(self, data):  # fit a batch
        feed_dict = {self.user: data['user'], self.item_pos: data['positive'], self.item_neg: data['negative'],
                     ...
                     self.ru1_p: data['ru1_p'], self.ru2_p: data['ru2_p'], self.ru3_p: data['ru3_p'],
                     self.eu1_p: data['eu1_p'], self.eu2_p: data['eu2_p'], self.eu3_p: data['eu3_p'],
                     self.cu1_p: data['cu1_p'], self.cu2_p: data['cu2_p'], self.cu3_p: data['cu3_p'],
                     self.lenu1_p: data['lenu1_p'], self.lenu2_p: data['lenu2_p'], self.lenu3_p: data['lenu3_p'],
                     self.ru1_n: data['ru1_n'], self.ru2_n: data['ru2_n'], self.ru3_n: data['ru3_n'],
                     self.eu1_n: data['eu1_n'], self.eu2_n: data['eu2_n'], self.eu3_n: data['eu3_n'],
                     self.cu1_n: data['cu1_n'], self.cu2_n: data['cu2_n'], self.cu3_n: data['cu3_n'],
                     self.lenu1_n: data['lenu1_n'], self.lenu2_n: data['lenu2_n'], self.lenu3_n: data['lenu3_n'],
    ...
    def get_random_block_from_data(self, train_data, batch_size):  # generate a random block of training data
        ...
        while len(user) < batch_size:
            ...
            # t1 = time()
            r0_temp, r1_temp, r2_temp, r3_temp, e1_temp, e2_temp, e3_temp, cnt0_temp, cnt1_temp, cnt2_temp, cnt3_temp, ru1_temp, ru2_temp, ru3_temp, cu1_temp, cu2_temp, cu3_temp = get_relational_data(
            ...
            ru1_p.append(ru1_temp)
            ru2_p.append(ru2_temp)
            ru3_p.append(ru3_temp)
            cu1_p.append(cu1_temp)
            cu2_p.append(cu2_temp)
            cu3_p.append(cu3_temp)
            ...
            ru1_n.append(ru1_temp)
            ru2_n.append(ru2_temp)
            ru3_n.append(ru3_temp)
            cu1_n.append(cu1_temp)
            cu2_n.append(cu2_temp)
            cu3_n.append(cu3_temp)
        ...
        lenu2_p = max(cu2_p)
        lenu3_p = max(cu3_p)
        for index in range(len(ru1_p)):
            if len(ru1_p[index]) < lenu1_p:
                ru1_p[index].extend(np.array([self.num_users]).repeat(lenu1_p - len(ru1_p[index])))
            if len(ru2_p[index]) < lenu2_p:
                ru2_p[index].extend(np.array([self.num_users]).repeat(lenu2_p - len(ru2_p[index])))
            if len(ru3_p[index]) < lenu3_p:
                ru3_p[index].extend(np.array([self.num_users]).repeat(lenu3_p - len(ru3_p[index])))
        lenu1_n = max(cu1_n)
        lenu2_n = max(cu2_n)
        lenu3_n = max(cu3_n)
        for index in range(len(ru1_n)):
            if len(ru1_n[index]) < lenu1_n:
                ru1_n[index].extend(np.array([self.num_users]).repeat(lenu1_n - len(ru1_n[index])))
            if len(ru2_n[index]) < lenu2_n:
                ru2_n[index].extend(np.array([self.num_users]).repeat(lenu2_n - len(ru2_n[index])))
            if len(ru3_n[index]) < lenu3_n:
                ru3_n[index].extend(np.array([self.num_users]).repeat(lenu3_n - len(ru3_n[index])))
        ...
        for i in range(len(eu1_p)):
            eu1_p[i] = np.array([1 / eu1_p[i]]).repeat(lenu1_p).tolist()
            # print(cu1_p)
            eu2_p[i] = np.array([1 / eu2_p[i]]).repeat(lenu2_p).tolist()
            eu3_p[i] = np.array([1 / eu3_p[i]]).repeat(lenu3_p).tolist()

            eu1_n[i] = np.array([1 / eu1_n[i]]).repeat(lenu1_n).tolist()
            eu2_n[i] = np.array([1 / eu2_n[i]]).repeat(lenu2_n).tolist()
            eu3_n[i] = np.array([1 / eu3_n[i]]).repeat(lenu3_n).tolist()
        ...
        return {'user': user, 'positive': positive, 'negative': negative, 'alpha': alpha,
                ...
                'ru1_p': ru1_p, 'ru2_p': ru2_p, 'ru3_p': ru3_p,
                'eu1_p': eu1_p, 'eu2_p': eu2_p, 'eu3_p': eu3_p,
                'cu1_p': cu1_p, 'cu2_p': cu2_p, 'cu3_p': cu3_p,
                'lenu1_p': lenu1_p, 'lenu2_p': lenu2_p, 'lenu3_p': lenu3_p,
                'ru1_n': ru1_n, 'ru2_n': ru2_n, 'ru3_n': ru3_n,
                'eu1_n': eu1_n, 'eu2_n': eu2_n, 'eu3_n': eu3_n,
                'cu1_n': cu1_n, 'cu2_n': cu2_n, 'cu3_n': cu3_n,
                'lenu1_n': lenu1_n, 'lenu2_n': lenu2_n, 'lenu3_n': lenu3_n,
    ...
    def train(self, Train_data):  # fit a dataset
        ...
        for epoch in range(self.epoch):
            ...
            now = datetime.datetime.now()
            f.write(now.strftime('%Y-%m-%d %H:%M:%S') + '\n')
            f.write("the total loss in %d th iteration is: %f, the attentions are %.4f, %.4f, %.4f, %.4f" % (
            epoch, total_loss, avearge[0], avearge[1], avearge[2], avearge[3]) + '\n')
            f.close()
    ...
    def get_scores_per_user(self, user_id):  # evaluate the results for an user context, return scorelist
        ...
        for i in range(int(batch_count)):
            ...
            lenu2_p = max(cu2_p)
            lenu3_p = max(cu3_p)
            # maxlenu_p = max(lenu1_p, lenu2_p, lenu3_p)
            for index in range(len(ru1_p)):
                if len(ru1_p[index]) < lenu1_p:
                    ru1_p[index].extend(np.array([self.num_users]).repeat(lenu1_p - len(ru1_p[index])))
                if len(ru2_p[index]) < lenu2_p:
            ...
            feed_dict = {self.user: user, self.item_pos: positive, self.alpha: alpha,
                         ...
                         self.ru1_p: ru1_p, self.ru2_p: ru2_p, self.ru3_p: ru3_p,
                         self.eu1_p: eu1_p, self.eu2_p: eu2_p, self.eu3_p: eu3_p,
                         self.cu1_p: cu1_p, self.cu2_p: cu2_p, self.cu3_p: cu3_p,
                         self.lenu1_p: lenu1_p, self.lenu2_p: lenu2_p, self.lenu3_p: lenu3_p,
